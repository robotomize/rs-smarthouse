...
pub trait DeviceInfoProvider {
    fn get_device_status(&self, _: &str, device: &DeviceTypes) -> Option<String>;
}
...
impl DeviceInfoProvider for OwningDeviceInfoProvider {
    fn get_device_status(&self, room: &str, device: &DeviceTypes) -> Option<String> {
        match device {
...
mod smart_socket_tests {
    ...
    fn test_smart_socket_tv() {
        ...
        assert_eq!(socket.get_device_status("Living", &TV), Some("State: On".to_string()));
    }

    #[test]
    fn test_smart_socket_lamp() {
        let socket = SmartSocket {};
        assert_eq!(socket.get_device_status("Living", &Lamp), Some("Luminosity: 70%".to_string()));
    }

    #[test]
    fn test_smart_socket_fridge() {
        let socket = SmartSocket {};
        assert_eq!(socket.get_device_status("Living", &Fridge), Some("220w".to_string()));
    }
}
...
mod smart_thermometer_tests {
    ...
    fn test_smart_thermometer_thermo() {
        ...
        assert_eq!(thermo.get_device_status("Living", &Thermo), Some("Temp: 20C".to_string()));
    }

    #[test]
    fn test_smart_thermometer_unknown() {
        let thermo = SmartThermometer {};
        assert_eq!(thermo.get_device_status("Living", &DeviceTypes::TV), None);
    }
}
